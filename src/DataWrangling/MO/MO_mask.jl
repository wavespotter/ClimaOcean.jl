...
"""
    MO_mask(architecture = CPU(); minimum_value = Float32(-1e5))

A boolean field where `true` represents a missing value in the MO dataset.
"""
function MO_mask(metadata, architecture = CPU(); 
                   data_field = MO_field(metadata; architecture, inpainting=nothing),
                   minimum_value = Float32(-1e5),
                   maximum_value = Float32(1e5))

    mask  = Field{location(data_field)...}(data_field.grid, Bool)

    # MO expresses missing values with values = 9.96921e+36
    _set_mask! = _set_MO_mask!
...
# Default
#MO_mask(arch::AbstractArchitecture=CPU()) = MO_mask(Metadata(:temperature, dataset=MO4Monthly()), arch)
MO_mask(arch::AbstractArchitecture=CPU()) = MO_mask(Metadata(:temperature, dataset=MODaily()), arch)

@kernel function _set_MO_mask!(mask, Tᵢ, minimum_value, maximum_value)
    i, j, k = @index(Global, NTuple)
    @inbounds mask[i, j, k] = (Tᵢ[i, j, k] < minimum_value) | (Tᵢ[i, j, k] > maximum_value) 
end

#@kernel function _set_MO4_mask!(mask, Tᵢ, args...)
#    i, j, k = @index(Global, NTuple)
#    @inbounds mask[i, j, k] = (Tᵢ[i, j, k] == 0) 
#end

"""
    MO_immersed_grid(metadata, architecture = CPU())
...
"""
function MO_immersed_grid(metadata, architecture = CPU())

    mask = MO_mask(metadata, architecture)
    grid = mask.grid
...
# Default
#MO_immersed_grid(arch::AbstractArchitecture=CPU()) = MO_immersed_grid(Metadata(:temperature, dataset=MO4Monthly()), arch)
MO_immersed_grid(arch::AbstractArchitecture=CPU()) = MO_immersed_grid(Metadata(:temperature, dataset=MODaily()), arch)
